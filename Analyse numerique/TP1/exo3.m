...
for xi=2:length(x)
    ...
    u(xi,1) = (1/dx)*quadv(g,xtilde(xi-1),xtilde(xi));
end
...
for i=2:length(x)-1
    M(i,i-1) = 0.5*alpha*(alpha+1);
    M(i,i) = 1-alpha.^2;
    M(i,i+1) = 0.5*alpha*(alpha-1);
end
M(1,length(x)) = 0.5*alpha*(alpha+1); M(1,1) = 1-alpha.^2; M(1,2) = 0.5*alpha*(alpha-1);
M(length(x),length(x)-1) = 0.5*alpha*(alpha+1); M(length(x),length(x)) = 1-alpha.^2; M(length(x),1) =0.5*alpha*(alpha-1);
