...
%application des conditions limites de Neumann

M(1,:) = 0; M(Nx+1,:) = 0; N(1,:) = 0; N(Nx+1,:) = 0; 
M(1,1) = -1/dx; M(1,2) = 1/dx; M(Nx+1,Nx) = -1/dx; M(Nx+1,Nx+1) = 1/dx;
N(1,1) = -1/dx; N(1,2) = 1/dx; N(Nx+1,Nx) = -1/dx; N(Nx+1,Nx+1) = 1/dx;
if k==1
    f(1,:) = 0; f(Nx+1,:) = 0;
elseif k==2
    f(1,:) = 1; f(Nx+1,:) = 0;
elseif k==3
    f(1,:) = -1/10; f(Nx+1,:) = -1/10;
end
u(1,1) = u(2,1)-dx*f(1,1); u(Nx+1,1) = u(Nx,1)+dx*f(Nx+1,1);
...
%mov = VideoWriter(strcat('./VideoNeumann',strcat(m,strcat('Cas',strcat(num2str(k),'.avi')))));
for i=1:length(t)
